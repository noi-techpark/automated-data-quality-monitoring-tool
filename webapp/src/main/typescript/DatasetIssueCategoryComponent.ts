...
export class DatasetIssueCategoryComponent extends CommonWebComponent
{
	...
	async refresh(data: {dataset_name: string, tot_records: number, failed_records: number, check_category: string, session_start_ts: string, test_dataset_id: number, check_name: string}, p_dataset_ids: string)
	{
		...
		view_details.onclick = () => {
			 ...
			 */
			location.hash = '#page=summary&test_dataset_id=' + data.test_dataset_id + '&test_dataset_ids=' + p_dataset_ids
		}
